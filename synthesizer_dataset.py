...
class SynthesizerDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bert_embeds = self.tokenizer(text, return_tensors="pt")
        text = text_to_sequence(text)
        text = torch.tensor(text, dtype=torch.int)
        # Convert the list returned by text_to_sequence to a numpy array
        return text, mel, embed.astype(np.float32), bert_embeds, index
...
def collate_synthesizer(batch, r, hparams):
    ...
    bert_embeds = [x[3] for x in batch]
    ...
    return chars, mel, embeds, bert_embeds, indices
